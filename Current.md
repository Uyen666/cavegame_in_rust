...
* **跨區塊全域探測與聯動更新 (Global Check & Remesh Propagation)**：面剔除判定完全拋棄局部邊界條件限制，系統依賴絕對座標全域查詢 `world.get_block_global(global_pos)`（直接讀取資料層 `ChunkEntry.palette`，零 ECS 查詢開銷）。當玩家在區塊交界處放置或破壞方塊時，系統會透過 6 向邊界偵測自動將相鄰區塊標記為 Dirty，確保跨區塊接縫的 Face Culling 即時無縫重算。
...
* **體素調色盤與存檔防禦 (Save Bloating Defense)**：使用 Palette Compression 將資料壓縮，髒區塊卸載時交由 IoTaskPool 異步寫入硬碟。若判定該區塊已被挖空退化為「純空氣」，不僅拒絕產生新存檔，還會在背景自動刪除硬碟上的歷史殘留檔案，從根源杜絕存檔無限膨脹與「歷史幽靈復原」。
