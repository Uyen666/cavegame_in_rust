...
* **2D 快捷列 UI 介面 (Hotbar HUD)**：在 `hud.rs` 中實作了完整的 2D UI 疊加層。於底部中央繪製 9 個方格，內部左上角配有 1~9 的微型數字索引，中央帶有 24x24 的 `UiImage` 紋理採樣預覽 (Texture Preview)，動態讀取 `assets/textures/` 下的材質圖檔（利用 `UiImage.color` 進行智慧降級：缺少圖檔時 Tint 成對應色塊，空氣則設為完全透明）。並透過 `Update` 排程與 `Changed<Player>` 篩選器動態追蹤 `selected_slot` 與對應的方塊種類。選中的方格會加粗邊框並以高對比亮黃色 (`srgb(1.0, 0.84, 0.0)`) 高亮顯示。對於純空氣 (`Air`) 則實施文字防空處理，保持 UI 極度乾淨。
